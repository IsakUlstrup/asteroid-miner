  ...
  setup() {
    ...
    onMounted(() => {
      ...
      space.value.addEventListener("mousedown", event => {
        if (ship.enabledLasers.length > 0) mining.value = true;
      });
    ...
    function mine(dt:number) {
      if (asteroids.length <= 0) return;
      if (ship.components[0].active && typeof miningTarget.value !== "undefined") miningTarget.value.hp -= ship.enabledLasers[0].power * dt * .01;
    }
